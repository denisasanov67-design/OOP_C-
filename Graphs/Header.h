...
public:
	Graph(){}	
	Graph(const char* file_name); // графф из файла 
	~Graph();
	...
	vector<vector<Node*>> connectedComponents(); // найти компоненты связанности 
...
class Node {
	...
	void addNeighbour(Node* neighbour); // добавить соседа 
	void removeNeighbour(Node* neighbour); // удалить соседа 
